...
import { DomainV2, DomainV2Attributes, DomainV2CreationAttributes } from '../models/DomainV2';
...
/**
 * DomainV2Repository 类
 */
export class DomainV2Repository {
  /**
   * 创建域名
   */
  async create(data: DomainV2CreationAttributes): Promise<DomainV2> {
    try {
      const domain = await DomainV2.create(data);
   ...
   * 通过 ID 查询域名（包含关联的配置）
   */
  async findById(id: number): Promise<DomainV2 | null> {
    try {
      return await DomainV2.findByPk(id, {
        include: [{
          model: Config,
          as: 'config',
        }],
   ...
   * 通过域名查询（包含关联的配置）
   */
  async findByDomain(domain: string): Promise<DomainV2 | null> {
    try {
      return await DomainV2.findOne({
        where: { domain },
        include: [{
          model: Config,
          as: 'config',
   ...
   * 查询所有域名（分页，包含关联的配置）
   */
  async findAll(pagination: Pagination): Promise<DomainV2[]> {
    try {
      const { page, pageSize } = pagination;
      const offset = (page - 1) * pageSize;

      return await DomainV2.findAll({
        limit: pageSize,
        offset,
        order: [['id', 'DESC']],
        include: [{
   ...
   * 更新域名
   */
  async update(id: number, data: Partial<DomainV2Attributes>): Promise<DomainV2 | null> {
    try {
      const domain = await DomainV2.findByPk(id);
      if (!domain) {
        return null;
      }
...
export default new DomainV2Repository();
