# RESTful API 规范分析报告

## 📋 当前 API 设计评估

### ✅ 符合 RESTful 规范的部分

#### 1. 资源命名（Resource Naming）
- ✅ 使用复数名词：`/domains`, `/configs`
- ✅ 使用小写字母
- ✅ 使用连字符分隔（如果需要）
- ✅ 资源层次清晰

#### 2. HTTP 方法使用（HTTP Methods）
- ✅ GET - 获取资源
- ✅ POST - 创建资源
- ✅ PUT - 更新资源
- ✅ DELETE - 删除资源

#### 3. HTTP 状态码（Status Codes）
- ✅ 200 - 成功获取/更新/删除
- ✅ 201 - 成功创建
- ✅ 400 - 请求验证失败
- ✅ 401 - 未认证
- ✅ 403 - 无权限
- ✅ 404 - 资源不存在
- ✅ 409 - 资源冲突

#### 4. 响应格式（Response Format）
- ✅ 统一使用 JSON
- ✅ 错误响应格式统一
- ✅ 包含适当的元数据（分页信息等）

---

## ⚠️ 不符合 RESTful 规范的部分

### 问题 1: 路径参数不一致 ❌

**当前设计**:
```
GET /api/v1/domains/{domain}     # 通过域名字符串获取
GET /api/v1/domains/id/{id}      # 通过 ID 获取
PUT /api/v1/domains/{id}         # 通过 ID 更新
DELETE /api/v1/domains/{id}      # 通过 ID 删除
```

**问题**:
- `/domains/{domain}` 和 `/domains/id/{id}` 路径冲突
- 如果域名是 "id"，会产生歧义
- 不符合 RESTful 的统一接口原则

**RESTful 最佳实践**:
资源应该通过唯一标识符（通常是 ID）访问，如果需要通过其他字段查询，应该使用查询参数。

---

### 问题 2: GET 请求混合了两种功能 ❌

**当前设计**:
```
GET /api/v1/domains?url=baidu.com    # 查询单个域名
GET /api/v1/domains                  # 获取列表
```

**问题**:
- 同一个端点根据查询参数返回不同类型的数据（单个对象 vs 列表）
- 响应格式不一致
- 违反了单一职责原则

**RESTful 最佳实践**:
- 列表查询和单个资源查询应该是不同的端点
- 或者统一返回数组格式（即使只有一个结果）

---

### 问题 3: 删除操作返回 200 而不是 204 ⚠️

**当前设计**:
```
DELETE /api/v1/domains/{id}
Response: 200 { "message": "域名删除成功" }
```

**RESTful 最佳实践**:
- 删除成功应该返回 `204 No Content`（无响应体）
- 或者返回 `200 OK` 并包含被删除的资源信息

---

### 问题 4: 认证端点不符合 RESTful 风格 ⚠️

**当前设计**:
```
POST /api/v1/auth/login
```

**问题**:
- `login` 是动词，不是资源
- RESTful 应该面向资源，而不是动作

**RESTful 最佳实践**:
```
POST /api/v1/sessions      # 创建会话（登录）
DELETE /api/v1/sessions    # 删除会话（登出）
```

或者使用 OAuth 2.0 标准：
```
POST /api/v1/token         # 获取令牌
```

---

## 🔧 改进建议

### 方案 A: 最小改动方案（推荐）

保持向后兼容，只修复最严重的问题。

#### 1. 修复路径冲突

**修改前**:
```
GET /api/v1/domains/{domain}     # 通过域名获取
GET /api/v1/domains/id/{id}      # 通过 ID 获取
```

**修改后**:
```
GET /api/v1/domains/{id}                    # 通过 ID 获取（标准 RESTful）
GET /api/v1/domains?domain=example.com      # 通过域名查询（查询参数）
GET /api/v1/domains?url=example.com         # 通过 URL 查询（保持兼容）
```

#### 2. 统一查询接口

**修改前**:
```
GET /api/v1/domains?url=baidu.com    # 返回单个对象
GET /api/v1/domains                  # 返回列表
```

**修改后**:
```
GET /api/v1/domains                          # 返回列表（分页）
GET /api/v1/domains?domain=baidu.com         # 返回列表（过滤，可能 0 或 1 个结果）
GET /api/v1/domains/{id}                     # 返回单个对象
```

#### 3. 优化删除响应

**修改前**:
```
DELETE /api/v1/domains/{id}
Response: 200 { "message": "域名删除成功" }
```

**修改后**:
```
DELETE /api/v1/domains/{id}
Response: 204 No Content
```

---

### 方案 B: 完全 RESTful 重构方案

完全符合 RESTful 规范，但会破坏向后兼容性。

#### 1. 资源路径设计

```
# Domains 资源
GET    /api/v1/domains              # 获取域名列表（支持过滤）
POST   /api/v1/domains              # 创建域名
GET    /api/v1/domains/{id}         # 获取单个域名
PUT    /api/v1/domains/{id}         # 更新域名
PATCH  /api/v1/domains/{id}         # 部分更新域名
DELETE /api/v1/domains/{id}         # 删除域名

# Configs 资源
GET    /api/v1/configs              # 获取配置列表
POST   /api/v1/configs              # 创建配置
GET    /api/v1/configs/{id}         # 获取单个配置
PUT    /api/v1/configs/{id}         # 更新配置
PATCH  /api/v1/configs/{id}         # 部分更新配置
DELETE /api/v1/configs/{id}         # 删除配置

# 认证资源（会话）
POST   /api/v1/sessions             # 创建会话（登录）
DELETE /api/v1/sessions             # 删除会话（登出）
GET    /api/v1/sessions/current     # 获取当前会话信息
```

#### 2. 查询参数设计

```
# 分页
GET /api/v1/domains?page=1&pageSize=20

# 过滤
GET /api/v1/domains?domain=example.com
GET /api/v1/domains?search=example

# 排序
GET /api/v1/domains?sort=createdAt&order=desc

# 字段选择
GET /api/v1/domains?fields=id,domain,homepage

# 关联资源
GET /api/v1/domains?include=config
GET /api/v1/domains/{id}?include=config
```

#### 3. 响应格式统一

```json
// 列表响应
{
  "data": [...],
  "meta": {
    "page": 1,
    "pageSize": 20,
    "total": 100,
    "totalPages": 5
  },
  "links": {
    "self": "/api/v1/domains?page=1",
    "first": "/api/v1/domains?page=1",
    "last": "/api/v1/domains?page=5",
    "next": "/api/v1/domains?page=2",
    "prev": null
  }
}

// 单个资源响应
{
  "data": {...}
}

// 错误响应
{
  "error": {
    "code": "VALIDATION_ERROR",
    "message": "请求数据验证失败",
    "details": [...]
  }
}
```

#### 4. HTTP 方法语义

- **GET**: 幂等，无副作用，可缓存
- **POST**: 非幂等，创建资源
- **PUT**: 幂等，完全替换资源
- **PATCH**: 幂等，部分更新资源
- **DELETE**: 幂等，删除资源

#### 5. 状态码使用

- **200 OK**: 成功（GET, PUT, PATCH）
- **201 Created**: 创建成功（POST），响应头包含 `Location`
- **204 No Content**: 删除成功（DELETE）
- **400 Bad Request**: 请求格式错误
- **401 Unauthorized**: 未认证
- **403 Forbidden**: 已认证但无权限
- **404 Not Found**: 资源不存在
- **409 Conflict**: 资源冲突
- **422 Unprocessable Entity**: 语义错误（如业务规则验证失败）
- **429 Too Many Requests**: 请求过于频繁
- **500 Internal Server Error**: 服务器错误

---

## 🎯 推荐实施方案

### 阶段 1: 修复严重问题（立即实施）

1. **修复路径冲突**
   - 移除 `/domains/id/{id}` 路径
   - 将 `/domains/{domain}` 改为查询参数方式
   - 保持 `/domains/{id}` 作为标准路径

2. **统一查询接口**
   - GET `/domains` 始终返回列表格式
   - 使用查询参数过滤：`?domain=xxx`
   - 单个资源通过 `/domains/{id}` 获取

3. **优化删除响应**
   - DELETE 操作返回 204 状态码
   - 不返回响应体

### 阶段 2: 增强功能（可选）

1. **添加 PATCH 支持**
   - 支持部分更新资源
   - 与 PUT 区分开来

2. **增强查询能力**
   - 支持更多过滤参数
   - 支持排序
   - 支持字段选择

3. **改进认证端点**
   - 将 `/auth/login` 改为 `/sessions`
   - 更符合 RESTful 风格

### 阶段 3: 版本管理（长期）

1. **引入 API 版本**
   - 当前保持 `/api/v1/`
   - 新版本使用 `/api/v2/`
   - 在 v2 中实现完全 RESTful 设计

2. **向后兼容**
   - v1 继续支持旧客户端
   - 逐步迁移到 v2
   - 设置 v1 废弃时间表

---

## 📊 对比表格

| 特性 | 当前设计 | RESTful 最佳实践 | 优先级 |
|------|---------|-----------------|--------|
| 资源命名 | ✅ 复数名词 | ✅ 复数名词 | - |
| HTTP 方法 | ✅ 正确使用 | ✅ 正确使用 | - |
| 路径参数 | ❌ 不一致 | ✅ 统一使用 ID | 🔴 高 |
| 查询参数 | ⚠️ 混合使用 | ✅ 清晰分离 | 🟡 中 |
| 删除响应 | ⚠️ 200 + body | ✅ 204 No Content | 🟢 低 |
| 认证端点 | ⚠️ 动词形式 | ✅ 资源形式 | 🟢 低 |
| 状态码 | ✅ 基本正确 | ✅ 完全正确 | 🟢 低 |
| 响应格式 | ⚠️ 不完全统一 | ✅ 完全统一 | 🟡 中 |
| HATEOAS | ❌ 不支持 | ⚠️ 可选 | 🟢 低 |

---

## 💡 实施建议

### 立即修复（高优先级）

修复路径冲突问题，这是最严重的 RESTful 违规：

```typescript
// 删除这个路由
router.get('/id/:id', ...)

// 修改这个路由为查询参数
router.get('/', async (req, res) => {
  const { domain, url, page, pageSize } = req.query;
  
  // 如果提供了 domain 或 url 参数，进行过滤查询
  if (domain || url) {
    const searchDomain = domain || url;
    const result = await domainService.findByDomain(searchDomain);
    // 返回列表格式（即使只有一个结果）
    return res.json({
      data: result ? [result] : [],
      pagination: { page: 1, pageSize: 1, total: result ? 1 : 0, totalPages: 1 }
    });
  }
  
  // 否则返回分页列表
  const result = await domainService.list({ page, pageSize });
  res.json(result);
});

// 保持标准的 ID 路由
router.get('/:id', ...)
```

### 渐进改进（中优先级）

1. 统一响应格式
2. 优化删除操作返回 204
3. 添加更多查询参数支持

### 长期规划（低优先级）

1. 引入 API v2
2. 实现完全 RESTful 设计
3. 添加 HATEOAS 支持

---

## 📚 RESTful 设计原则总结

1. **资源导向**: 使用名词而不是动词
2. **统一接口**: 使用标准 HTTP 方法
3. **无状态**: 每个请求包含所有必要信息
4. **可缓存**: GET 请求应该可缓存
5. **分层系统**: 客户端不需要知道是否直接连接到服务器
6. **按需代码**: 可选，服务器可以返回可执行代码

---

## 🔗 参考资源

- [RESTful API 设计指南](https://restfulapi.net/)
- [Microsoft REST API Guidelines](https://github.com/microsoft/api-guidelines)
- [Google API Design Guide](https://cloud.google.com/apis/design)
- [HTTP 状态码](https://httpstatuses.com/)
